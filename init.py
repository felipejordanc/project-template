...
print("- A local folder where you can locally store data that you don't want to have in your remote data folder (e.g. large files that are used in intermediate steps during development). You should programatically create the necessary subdirectories and files if you want your colaborators to be able to reproduce your results with the shared code. This folder will be ignored by git.\n")
...
if os.path.exists('remote'):
    ...
    while True:
        ...
        if not os.path.exists(target_dir):
            print('{0} was not found in your system. You may include the path by dragging the directory into the terminal, but you have to delate any extra space added at the end.'.format(target_dir))
            continue
        else:
